  ...
  export function renderCategories(categoryData, preSelectedList) {
    selectUl.innerHTML = '';
    ...
    categoryData.forEach(category => {
      ...
      preSelectedList.forEach(listId => {
        if (listId == id) { 
          selectACategory(li);
        }
      })
  ...
  function addTemporaryListeners() {   
  ...
  function selectACategory(list) { 
    ...
    const name = list.children[2].innerHTML;
    const emoji = list.children[1].innerHTML;
